...
const BidTable = ({ bids }: { bids: [string, string][] }) => {
    ...
    const bidsWithTotal: [string, string, number][] = relevantBids.map(([price, quantity]) => [
        price, quantity, currentTotal += Number(quantity)
    ])
    const maxTotal = relevantBids.reduce((acc, [_, quantity]) => acc + Number(quantity), 0)
    return (
        <div>
            {bidsWithTotal.map(([price, quantity, total]) => <Bid key={price} price={price} quantity={quantity} total={total} maxTotal={maxTotal} />)}
        </div>
    )
}
